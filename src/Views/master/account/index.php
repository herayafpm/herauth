    ...
    <div class="col-md-12">
        <div class="card">
            <div class="card-header">
                <!-- <a role="button" class="btn btn-sm btn-success" href="<?= $url_add ?>"><?= lang("Label.add") . " " . lang("Label.admin") ?></a> -->
            </div>
            <div class="card-body">
...
<script>
    ...
    $(document).ready(function() {
            ...
            "columns": [{
                ...
                },
                {
                    ...
                    "render": function(dt, type, row, meta) { // Tampilkan kolom aksi
                        var html = '';
                        var url_group = decodeURIComponent("<?= $url_group ?>").format(row.id);
                        var url_edit = decodeURIComponent("<?= $url_edit ?>").format(row.id);
                        html += `
                            <a role="button" class="btn btn-sm btn-info" href="${url_group}">
        ...
        $("#tableMaster").on('click', '.deleteData', function() {
            var id = $(this).data('id')
            Swal.fire({
                title: herlangjs('Label.confirm')+" "+herlangjs('Label.delete')+" "+herlangjs('Label.admin'),
                icon: 'warning',
        ...
        $("#tableMaster").on('click', '.restoreData', function() {
            var id = $(this).data('id')
            Swal.fire({
                title: herlangjs('Label.confirm')+" "+herlangjs('Label.restore')+" "+herlangjs('Label.admin'),
                icon: 'warning',
                showCancelButton: true,
        ...
        $("#tableMaster").on('click', '.purgeData', function() {
            var id = $(this).data('id')
            Swal.fire({
                title: herlangjs('Label.confirm')+" "+herlangjs("Label.purge")+" "+herlangjs('Label.admin'),
                icon: 'warning',
                showCancelButton: true,
                confirmButtonColor: '#d33',
                cancelButtonColor: '#3085d6',
